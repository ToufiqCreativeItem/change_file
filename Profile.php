...
use App\Models\{Stories, Posts, Comments, Feeling_and_activities, CommonModels, Live_streamings, Users, Friendships, Media_files, Albums, User};
...
class Profile extends Controller
{
    ...
    function album($action_type, Request $request)
    {
        ...
        if ($action_type == 'form') {
            ...
            if ($request->thumbnail) {
                $file_name = random(40) . '.' . $request->thumbnail->extension();

                //Image optimization
                Image::make($request->thumbnail->path())->orientate()->resize(800, null, function ($constraint) {
                    $constraint->upsize();
                    $constraint->aspectRatio();
                })->save(public_path() . '/storage/thumbnails/album/' . $file_name);

                //Ultra Image optimization
                Image::make($request->thumbnail->path())->orientate()->resize(150, null, function ($constraint) {
                    $constraint->upsize();
                    $constraint->aspectRatio();
                })->save(public_path() . '/storage/thumbnails/album/optimized/' . $file_name);
            ...
            $response = array('hideCustomModal' => 1, 'appendAfterElement' => '#profile-album-row .col-create-album:first-child', 'content' => $album_view);
    ...
    function about($action_type, Request $request)
    {
        ...
        if ($action_type = 'update') {
            ...
            $response = array('alertMessage' => get_phrase('Your bio updated'), 'hideElem' => '.edit-bio-form', 'showElem' => '.edit-bio-btn', 'elemSelector' => '.my-about', 'content' => nl2br($request->about));
        }
    ...
    function upload_photo($photo_type, Request $request)
    {
        if ($photo_type == 'cover_photo') {
            ...
            //Upload image
            $file_name = random(40) . '.' . $request->cover_photo->getClientOriginalExtension();

            //Image optimization
            Image::make($request->cover_photo->path())->orientate()->resize(1120, null, function ($constraint) {
                $constraint->upsize();
                $constraint->aspectRatio();
            })->save(uploadTo('cover_photo') . $file_name);


            //Ultra Image optimization
            Image::make($request->cover_photo->path())->orientate()->resize(200, null, function ($constraint) {
                $constraint->upsize();
                $constraint->aspectRatio();
            })->save(uploadTo('cover_photo/optimized') . $file_name);
    ...
    function update_profile(Request $request)
    {
        ...
        if ($request->profile_photo && !empty($request->profile_photo)) {
            ...
            // Validate the input and return correct response
            $file_name = random(40) . '.' . $request->profile_photo->extension();

            //Image optimization
            Image::make($request->profile_photo->path())->orientate()->resize(800, null, function ($constraint) {
                $constraint->upsize();
                $constraint->aspectRatio();
            })->save(public_path() . '/storage/userimage/' . $file_name);

            //Ultra Image optimization
            Image::make($request->profile_photo->path())->orientate()->fit(150, 150, function ($constraint) {
                $constraint->upsize();
                $constraint->aspectRatio();
            })->save(public_path() . '/storage/userimage/optimized/' . $file_name);
